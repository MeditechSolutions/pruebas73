    ...
    def mercadopago_form_generate_values(self, values):
        ...
        if MPago:
            ...
            if acquirer.state=="enabled":
                sandbox = MPago.sandbox_mode(False)
            elif acquirer.state=="disabled":
                return {}
            else:
                sandbox = MPago.sandbox_mode(True)

            sandbox = MPago.sandbox_mode()
            if sandbox :
                MPago = mercadopago.MP(acquirer.mercadopago_api_access_token)
                sandbox = MPago.sandbox_mode(True)
            MPagoToken = MPago.get_access_token()

            if (MPagoToken and acquirer.mercadopago_api_access_token != MPagoToken) :
    ...
    def _mercadopago_get_data(self, payment_id=None, reference=None ):
        ...
        for acquirer in self:
            ...
            if MPago:
                ...
                #search_uri = sandbox and '/sandbox' or ''
                search_uri = ''
                if (reference):
                    search_uri = search_uri+'/v1/payments/search?'+'external_reference='+reference+'&access_token='+acquirer.mercadopago_api_access_token
                else:
                    search_uri = search_uri+'/v1/payments/'+str(payment_id)+'?access_token='+acquirer.mercadopago_api_access_token
